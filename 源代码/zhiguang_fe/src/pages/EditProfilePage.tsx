        ...
        >
            <form className={styles.pageCard} onSubmit={onSubmit}>
                <SectionHeader
                    ...
                    actions={<>
                        <button type="button" className="ghost-button" onClick={() => navigate("/profile")}>返回
                        </button>
                        <button type="submit" className="ghost-button"
                                disabled={isSaving}>{isSaving ? "保存中..." : "保存修改"}</button>
                    </>}
                />

                <div className={styles.grid}>
                    <div className={styles.avatarPanel}>
                        <div className={styles.avatarPreview} onClick={onAvatarClick} role="button"
                             aria-label="点击上传头像" tabIndex={0}>
                            {avatarUrl ? (
                        ...
                        {/* 隐藏的文件输入，由点击头像触发 */}
                        <input ref={fileInputRef} id="avatar" type="file" accept="image/*" onChange={onAvatarFileChange}
                               style={{display: "none"}}/>
                    ...
                    <div className={styles.fieldGroup}>
                        <div className={styles.field}>
                            ...
                            <input id="nickname" className={styles.input} value={nickname}
                                   onChange={e => setNickname(e.target.value)} placeholder="填写你的昵称"/>
                        </div>
                        <div className={styles.field}>
                            <label className={styles.label} htmlFor="phone">手机</label>
                            <input id="phone" className={styles.input} value={phone}
                                   onChange={e => setPhone(e.target.value)} placeholder="绑定手机号方便联系"/>
                        </div>
                        <div className={styles.field}>
                            <label className={styles.label} htmlFor="zgId">知光 ID</label>
                            <input id="zgId" className={styles.input} value={zgId}
                                   onChange={e => setZgId(e.target.value)} placeholder="用于个性化主页地址"/>
                        </div>
                        <div className={styles.field}>
                        ...
                        <div className={styles.field}>
                            ...
                            <input id="birthday" className={styles.input} type="date" value={birthday}
                                   onChange={e => setBirthday(e.target.value)}/>
                        </div>
                        <div className={styles.field}>
                            <label className={styles.label} htmlFor="school">学校/机构</label>
                            <input id="school" className={styles.input} value={school}
                                   onChange={e => setSchool(e.target.value)} placeholder="填写你的所在学校或机构"/>
                        </div>
                        ...
                        <div className={`${styles.field} ${styles.fullWidth}`}>
                            ...
                            <textarea id="bio" className={styles.textarea} value={bio}
                                      onChange={e => setBio(e.target.value)} placeholder="介绍一下自己..."/>
                        </div>
                    </div>
                </div>
